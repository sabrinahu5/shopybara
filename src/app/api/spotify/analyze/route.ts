...
function formatMusicData(artists: any[], tracks: any[]) {
  ...
  try {
    ...
    const vibeAnalysis = await openai.chat.completions.create({
      messages: [
        {
          role: "system",
          content: "You are an expert at analyzing music taste and determining personality traits and aesthetic preferences."
        },
        {
          role: "user",
    ...
    const roomCuration = await openai.chat.completions.create({
      messages: [
        {
          role: "system",
          content: "You are an expert interior designer who specializes in creating cohesive, personalized spaces that reflect individual style."
        },
        {
          role: "user",
